...
if (isset($_SESSION['mensaje_accion_usuario'])) {
    $mensaje_exito = $_SESSION['mensaje_accion_usuario'];
    unset($_SESSION['mensaje_accion_usuario']); 
}
if (isset($_SESSION['error_accion_usuario'])) {
    $mensaje_error = $_SESSION['error_accion_usuario'];
    unset($_SESSION['error_accion_usuario']);
}

if (!isset($pdo)) {
    // Sobrescribir mensaje de error solo si no hay uno de acción previo
    if (empty($mensaje_error)) $mensaje_error = "Error crítico: La conexión a la base de datos no está disponible.";
} else {
    try {
        ...
        $sql_count = "SELECT COUNT(*) 
                      ...
                      WHERE u.id_rol != 1"; // Excluir administradores
        ...
        // --- CONSULTA PRINCIPAL PARA OBTENER USUARIOS (CORREGIDA) ---
        ...
        if (empty($mensaje_error)) { // Mostrar este error solo si no hay uno de acción previo
...
// de la respuesta anterior donde se añadieron los botones de Editar/Borrar.
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .controles-tabla form{display:flex;gap:10px;flex-grow:1;max-width:400px}
        .controles-tabla input[type="text"]{padding:8px 10px;border:1px solid #ccc;border-radius:5px;font-size:.9em;flex-grow:1}
        .controles-tabla button[type="submit"]{padding:8px 15px;background-color:#5cb85c;color:white;border:none;border-radius:5px;cursor:pointer;font-size:.9em}
        ...
        .tabla-datos .acciones a {
            display:inline-block;padding:5px 8px;margin-right:5px;margin-bottom:5px;
        ...
        @media (max-width:768px){
            ...
            .controles-tabla form { max-width:none; } /* Permitir que el form ocupe ancho en móvil */
            ...
            .tabla-datos .acciones a { display: block; margin-bottom: 5px; width: 100%; box-sizing: border-box; text-align:center;}
        }
                    ...
                    <?php foreach ($usuarios as $usuario): ?>
                        <tr>
                            ...
                            <td class="acciones">
                                ...
                                <?php if ($_SESSION['id_usuario'] !== $usuario['id_usuario']): // No permitir borrarse a sí mismo ?>
                                <a href="admin_delete_user.php?id_usuario=<?php echo htmlspecialchars($usuario['id_usuario']); ?>&pagina=<?php echo $pagina_actual; ?>&buscar=<?php echo urlencode($termino_busqueda); ?>" 
                                   class="btn-borrar" 
                                   onclick="return confirm('¿Estás realmente seguro de que deseas eliminar a este usuario (<?php echo addslashes(htmlspecialchars($usuario['nombre'])); ?>)? Esta acción no se puede deshacer.');">
                                   Borrar
                                </a>
                                <?php endif; ?>
                            </td>
            ...
            <?php if ($total_paginas > 1): ?>
                <div class="paginacion">
                    <?php if ($pagina_actual > 1): ?>
                        <a href="?pagina=<?php echo $pagina_actual - 1; ?>&buscar=<?php echo urlencode($termino_busqueda); ?>">Anterior</a>
                    <?php else: ?>
                    ...
                    if ($inicio_rango > 1) {
                        echo '<a href="?pagina=1&buscar=' . urlencode($termino_busqueda) . '">1</a>';
                        if ($inicio_rango > 2) { echo '<span>...</span>'; }
                    }
                    for ($i = $inicio_rango; $i <= $fin_rango; $i++):
                        if ($i == $pagina_actual): echo '<span class="actual">' . $i . '</span>';
                        else: echo '<a href="?pagina=' . $i . '&buscar=' . urlencode($termino_busqueda) . '">' . $i . '</a>'; endif;
                    endfor;
                    if ($fin_rango < $total_paginas) {
                        if ($fin_rango < $total_paginas - 1) { echo '<span>...</span>'; }
                        echo '<a href="?pagina=' . $total_paginas . '&buscar=' . urlencode($termino_busqueda) . '">' . $total_paginas . '</a>';
                    }
                    ?>

                    <?php if ($pagina_actual < $total_paginas): ?>
                        <a href="?pagina=<?php echo $pagina_actual + 1; ?>&buscar=<?php echo urlencode($termino_busqueda); ?>">Siguiente</a>
                    <?php else: ?>
